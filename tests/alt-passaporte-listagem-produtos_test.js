...
describe('alt.passaporte-listagem-produtos', function() {
  ...
  var URL_PASSAPORTE_PRODUTOS_HABILITADOS = BASE_URL_PASSAPORTE + '/passaporte-rest-api/rest/produtos/habilitados';
  ...
  describe('getProdutos', function() {
    ...
    it('deve buscar as informações corretamente', function() {
      ...
      _httpBackend.expectGET(URL_PASSAPORTE_PRODUTOS).respond(200, {habilitados: [], disponiveis: []});
  ...
  describe('ordenaPorNome', function() {
    it('deve ordenar os produtos por nome - não existe passaporte ou passaporte admin', function() {
      ...
      expect(_resultado.habilitados[2]).toEqual({nome: 'c'});
      expect(_resultado.habilitados[3]).toEqual({nome: 'x'});
    });

    it('deve ordenar os produtos por nome - existe passaporte, mas não passaporte admin', function() {
      var _produtos = {
        habilitados: [
          {nome: 'c'},
          {nome: 'x'},
      ...
      expect(_resultado.habilitados[2]).toEqual({nome: 'b'});
      expect(_resultado.habilitados[3]).toEqual({nome: 'c'});
      expect(_resultado.habilitados[4]).toEqual({nome: 'x'});
    });

    it('deve ordenar os produtos por nome - existe passaporte admin, mas não passaporte', function() {
      var _produtos = {
        habilitados: [
          {nome: 'c'},
          {nome: 'x'},
    ...
    it('deve ordenar os produtos por nome - existem ambos passaporte admin e passaporte', function() {
      ...
      expect(_resultado.habilitados[2]).toEqual({nome: 'a'});
  ...
  describe('getProdutosHabilitados', function() {
    it('deve tentar buscar os produtos, mas o servidor retorna erro', function() {
      spyOn(_AltPassaporteListagemProdutosService._altPassaporteAuthorizationInfoService, 'getToken').and.callFake(function() {
        return _q.when(TOKEN);
      });

      _httpBackend.expectGET(URL_PASSAPORTE_PRODUTOS_HABILITADOS).respond(400);
    ...
    it('deve buscar as informações corretamente', function() {
      ...
      _httpBackend.expectGET(URL_PASSAPORTE_PRODUTOS_HABILITADOS).respond(200, {habilitados: [], disponiveis: []});
    ...
    it('deve buscar as informações corretamente e fazer o parse das urls com os tokens - urlAutenticacao sem query string', function() {
      ...
      _httpBackend.expectGET(URL_PASSAPORTE_PRODUTOS_HABILITADOS).respond(200, _produtoWrapper);
